...
/-!
# Exact physical block dilation

The row type `Fin k × Fin n` is literally `k` physical copies of `R^n`.
This file proves the first and most important part of `lem:dilation`: the
stack defining `\widehat V` has exactly orthonormal columns.  The proof is the finite
telescoping form of the iterated Paige isometry.
-/
...
/-- An explicitly entrywise finite matrix sum.  Keeping the entry formula
visible makes block-stack calculations easy to audit. -/
def matFinSum {m n k : ℕ} (A : Fin k → Mat m n) : Mat m n :=
  fun i j => ∑ r, A r i j

/-- Stack a family of `n`-by-`k` blocks down `k` physical copies. -/
def stackBlocks (B : Fin k → Mat n k) : Matrix (Fin k × Fin n) (Fin k) ℝ :=
  fun ri j => B ri.1 ri.2 j

/-- The physical matrix `\widehat V=col(VC,VCK,...,VCK^{k-1})`. -/
def physicalX (V : Mat n k) (C K : Mat k k) :
    Matrix (Fin k × Fin n) (Fin k) ℝ :=
  stackBlocks fun r => V * C * K ^ r.1

/-- Gram matrix of a block stack is the sum of the block Gram matrices. -/
...
/-- A single summand in the physical Gram matrix is a telescoping
difference. -/
...
/-- First orthogonality identity in `lem:dilation`: the nilpotent physical
stack has exactly orthonormal columns. -/
...
/-! ## The one-step boundary column -/

/-- Moving a matrix across a Euclidean dot product transposes it. -/
...
/-- The coefficient and physical remainders in `eq:CKsy`. -/
...
/-- The off-diagonal block in the block identity of `lem:isometry` is zero. -/
...
/-- The lower-right entry in the block identity of `lem:isometry`. -/
...
/-! ## Iterating the boundary column -/

/-- Block `r` of the physical boundary vector; this definition also covers
`k=1` without a separate convention. -/
def physicalPBlock (V : Mat n k) (C K : Mat k k) (s : Vec k) (y : Vec n)
    (r : ℕ) : Vec n :=
  if r = 0 then y else (V * C * K ^ (r - 1)) *ᵥ s

/-- The physical boundary vector `\widehat v` in `lem:dilation`. -/
def physicalP (V : Mat n k) (C K : Mat k k) (s : Vec k) (y : Vec n) :
    (Fin k × Fin n) → ℝ :=
  fun ri => physicalPBlock V C K s y ri.1 ri.2

/-- The shifted telescope needed for `Xᵀp=0`. -/
...
/-- Every noninitial block contribution to `Xᵀp` is one difference in the
finite telescope. -/
...
/-- A squared physical block norm is one difference in the norm telescope. -/
...
/-- Vector-valued telescope proving the middle identity in
`lem:dilation`. -/
...
/-- The physical stack is orthogonal to its boundary vector, the second
identity in `lem:dilation`. -/
...
/-- Scalar telescope for the squared norm of the physical boundary. -/
...
/-- The boundary-vector norm inequality in `lem:dilation`. -/
...
/-! ## The first physical column -/

/-- The first coordinate vector, indexed from zero in Lean. -/
...
/-- The block vector in the first-column conclusion of `lem:dilation`. -/
def physicalFirst (hk : 0 < k) (V : Mat n k) : (Fin k × Fin n) → ℝ :=
  fun ri => if ri.1.1 = 0 then V ri.2 ⟨0, hk⟩ else 0

/-- Only the first physical copy of the first column is occupied. -/
...
/-! ## Dimension-free norm bounds for the triangular factors -/
...
/-- The `K` norm bound in `lem:isometry`. -/
...
/-- The `C` norm bound in `lem:isometry`. -/
