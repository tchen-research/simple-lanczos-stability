...
/-!
# Euclidean vectors and spectral-norm matrices

The notation in `finite_precision_lanczos.tex` uses Euclidean norms for
vectors and operator norms for matrices.  Mathlib's ordinary norm on a
function is not the Euclidean norm, so this file makes the conversion
explicit and gives the few bridge lemmas used throughout the formalization.
-/
...
/-- A real finite matrix equipped with Mathlib's Euclidean operator norm. -/
abbrev Mat (m n : ℕ) := Matrix (Fin m) (Fin n) ℝ

/-- Coordinate representation of a real vector. -/
...
/-- Euclidean norm on an arbitrary finite coordinate type.  This is used for
the physical product space `Fin k × Fin n` in `lem:dilation`. -/
noncomputable def coordNorm {ι : Type*} [Fintype ι] (x : ι → ℝ) : ℝ :=
  ‖(WithLp.toLp 2 x : EuclideanSpace ℝ ι)‖

/-- Euclidean inner product, written as a coordinate dot product. -/
...
/-- Cauchy--Schwarz for coordinate vectors. -/
...
/-- A matrix acts nonexpansively up to its Euclidean operator norm. -/
lemma vnorm_mulVec_le (A : Mat m n) (x : Vec n) :
    vnorm (A *ᵥ x) ≤ ‖A‖ * vnorm x := by
  simpa [vnorm] using
    A.l2_opNorm_mulVec (WithLp.toLp 2 x : EuclideanSpace ℝ (Fin n))

/-- Arbitrary-index version of Euclidean operator-norm control. -/
...
/-- Submultiplicativity of the Euclidean operator norm. -/
lemma norm_mul_le (A : Mat m n) (B : Mat n p) : ‖A * B‖ ≤ ‖A‖ * ‖B‖ :=
  Matrix.l2_opNorm_mul A B

/-- Transposition preserves the Euclidean operator norm. -/
...
/-- Matrix whose `j`th column is `q j`. -/
...
/-- Continuous Euclidean linear map represented by a matrix. -/
...
/-- The same continuous linear map for arbitrary finite row/column index
types, used by the physical product space. -/
...
/-- Triangle inequality for a finite sum of coordinate vectors. -/
...
/-- The Euclidean norm is bounded by the coordinate `ell_1` norm. -/
...
/-- Multiplication by a column stack is the corresponding vector sum. -/
...
/-- Auditable columnwise operator-norm estimate.  This intentionally uses a
sum rather than a hidden Frobenius-norm conversion. -/
...
/-- Dimension-explicit conversion from a uniform entry bound to operator
norm. -/
