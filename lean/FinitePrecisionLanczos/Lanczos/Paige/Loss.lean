...
/-!
# Paige loss-of-orthogonality identity

This file isolates the exact quadratic-form identity used in the proof of
`thm:paige`, so its index conventions can be compared directly with the PDF.
-/
...
/-- Last coordinate of a nonempty `k`-step prefix. -/
def lastIndex (hk : 0 < k) : Fin k := ⟨k - 1, by omega⟩

/-- Raw boundary residual `β_k v_{k+1}`. -/
def rawBoundary : Vec n := L.β k • L.v (k + 1)

/-- Physical Ritz vector `x=V_k y` from the paragraph preceding
`eq:ritz-residual-identity`. -/
def ritzVector (y : Vec k) : Vec n := L.Vmat *ᵥ y

/-- Sum of the two strict-upper-triangular error quadratic forms in the
proof of `thm:paige`. -/
def eta (y : Vec k) : ℝ :=
  -vdot y (L.upG *ᵥ y) + vdot y (L.upQ *ᵥ y)

/-- Weighted local-product diagonal sum in the proof of `thm:paige`. -/
def paigeSum (y : Vec k) : ℝ :=
  ∑ j : Fin k, (y j) ^ 2 * (L.p (j.1 + 1) - L.p j.1)

/-- The boundary matrix acts only on the last coordinate. -/
...
/-- Projecting the boundary gives the physical scalar bounded in
`thm:paige`. -/
...
/-- Quadratic form of `N_k`, with the sign used in the PDF. -/
...
/-- Exact sandwiched upper-commutator identity used in `thm:paige`, with no
additional smallness assumption. -/
