...
/-!
# Ritz residual identities

This file is the formal counterpart of `eq:ritz-residual-identity`,
`eq:ritz-residual-bound`, and `eq:dichotomy` in
`finite_precision_lanczos.tex`.
-/
...
/-- `eq:ritz-residual-identity`: the exact physical residual obtained by
applying the stacked governing relation to an eigenvector of `T_k`. -/
...
/-- When the last recurrence coefficient is nonzero, the returned boundary
vector has the near-unit estimate from `ass:normalization`. -/
...
/-- Norm form of the last-column estimate used in
`eq:ritz-residual-bound`. -/
...
/-- `eq:ritz-residual-bound`, retaining exactly the two terms displayed in
the PDF. -/
...
/-- `eq:dichotomy`: division is performed only under the displayed
nonzero-residual hypothesis. -/
