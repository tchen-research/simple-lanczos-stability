...
/-!
# Paige descent for leading Lanczos prefixes

This file connects the finite scalar selection theorem to the actual leading
principal matrices of a `LanczosRun`.  Its first milestone is the exact,
division-free formula `eq:leading-coefficient`.
-/
...
/-- The actual leading-prefix length represented by `s : Fin (t-1)`. -/
...
/-- The concrete run at a proper leading prefix of `t ≤ k`. -/
...
/-- Symmetric-matrix witness for a leading Ritz matrix. -/
...
/-- Indexed Ritz value of the prefix `T_{s+1}`. -/
...
/-- Corresponding member of Mathlib's orthonormal eigenbasis. -/
...
/-- The first `s+1` rows of the larger Ritz eigenvalue equation. -/
...
/-- `eq:leading-coefficient`, in exactly its division-free form. -/
...
/-! ### The quantities in `eq:rho-expansion` -/
...
/-- `eq:leading-coefficient` in the uniform scalar-data notation consumed by
the selection lemma. -/
...
/-- The paper's prefix Paige product, bounded uniformly by the full-run
explicit representative of `mathcal E_*`. -/
...
/-- First equality of `eq:rho-expansion`: group the strict upper Gram
quadratic form by its columns. -/
...
/-- Expansion of a coordinate prefix in Mathlib's orthonormal eigenbasis.
The coefficient is exactly the `c_r^{(s)}` used in the PDF. -/
...
/-- The second equality in `eq:rho-expansion`: expand the physical prefix
vector in the orthonormal Ritz basis. -/
...
/-- Full `eq:rho-expansion`, with both finite sums and all prefix indices
made explicit. -/
...
/-! ### The aggregate Cauchy--Schwarz estimate -/

/-- Restricting to leading coordinates cannot increase squared Euclidean
norm. -/
...
/-- The coordinates following all proper prefixes are a subvector of `y`. -/
...
/-- Parseval identity for the coefficients `c_r^{(s)}`. -/
...
/-- Cauchy--Schwarz within one prefix:
`sum_r |c_r^{(s)}| ≤ sqrt(s)`. -/
...
/-- Auxiliary form of the elementary index sum. -/
...
/-- Elementary dimension sum used in the outer Cauchy--Schwarz estimate. -/
...
/-- The two finite sums in the proof of `lem:descent` are together at most
`2t`, exactly as in the displayed estimate in the PDF. -/
...
/-- `lem:descent`, with the explicit full-run
Paige scale `6000 k³ eps ‖H‖` substituted for `mathcal E_*`. -/
