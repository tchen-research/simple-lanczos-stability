...
/-!
# Prefixes of a fixed run

The PDF's descent argument applies the Paige theorem to every leading
Lanczos prefix.  This file verifies that restricting a completed run to
`t ≤ k` preserves every field of the arithmetic model, including the
smallness and returned-vector hypotheses.
-/
...
/-- The first `t` steps of a completed run, as a run in its own right. -/
