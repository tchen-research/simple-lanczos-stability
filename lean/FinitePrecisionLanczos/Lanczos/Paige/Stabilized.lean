...
/-!
# Localization of stabilized Ritz values

This file formalizes `thm:stabilized`.  As in the
PDF, the proof descends only while the Gram loss is large, and closes with the
ordinary residual-to-spectrum estimate at the first small-loss prefix.
-/
...
/-- Explicit full-run representative of `Psi_k=8k mathcal E_*`, matching
the choice in the proof of `lem:descent`. -/
...
/-- A prefix displacement `Psi_t` is bounded by the full-run `Psi_k`. -/
...
/-- The norm lower bound used in the residual proof, stated with the coarse
constant that makes the final coefficient `3` immediate. -/
...
/-- Small-loss stopping branch of `thm:stabilized`. -/
...
/-- Inductive max-form estimate for every leading prefix.  This is the
explicit invariant used to prove the linear statement in `thm:stabilized`. -/
...
/-- Max-form estimate at the completed run. -/
...
/-- `thm:stabilized`, with every hidden polynomial made explicit.  The
first term is the current scalar residual estimate; the remaining two
terms are fixed polynomials in `k` times `eps ‖H‖`. -/
...
/-- Stabilized specialization of `thm:stabilized`. -/
...
/-- Terminal specialization: if `beta_k=0`, every Ritz value is stabilized. -/
...
/-- Eigenvalue-enumeration form of the terminal specialization in
`thm:stabilized`. -/
