...
/-!
# Explicit bounds for the Paige boundary product

The constants are deliberately coarse and polynomial: the PDF suppresses
precisely these factors with `\lesssim_k` in `thm:paige`.
-/
...
/-- A unit coordinate vector has every coordinate bounded by one. -/
...
/-- Coarse but dimension-explicit entrywise quadratic-form estimate. -/
...
/-- Every entry of `T_k` has the coefficient-envelope scale. -/
...
/-- Entry formula for the diagonal-defect commutator. -/
...
/-- Entrywise bound for `T_kD_k-D_kT_k`. -/
...
/-- Entrywise form of the final bound in `lem:gram-commutator`. -/
...
/-- Explicit bound for the two strict-upper-triangular error forms in the
proof of `thm:paige`. -/
...
/-- Uniform adjacent-product bound including the phantom `p_0=0`. -/
...
/-- Explicit bound for the local-product diagonal sum in the proof of
`thm:paige`. -/
...
/-- `thm:paige`, with explicit constant `6000 k³`. -/
