...
This file formalizes `thm:containment` in the PDF.  The proof is split into
the same auditable pieces as the paper: the exact Ritz-vector norm identity,
the small-loss Rayleigh argument, and the large-loss descent to a shorter
leading prefix.
-/
...
/-- Explicit representative of the paper's Paige scale `mathcal E_*`. -/
noncomputable def paigeScale : ℝ := (6000 * (k : ℝ) ^ 3) * L.ε * ‖L.H‖

/-- Explicit representative of the block-recurrence error `‖F_k‖`. -/
noncomputable def recurrenceScale : ℝ := (700 * (k : ℝ) ^ 2) * L.ε * ‖L.H‖

/-- Radius produced by at most `t-1` Paige descents and one Rayleigh step. -/
...
/-- The arithmetic assumptions themselves force a nonempty ambient space. -/
...
/-- Exact Gram expansion for the physical Ritz vector.  This is the identity
used immediately after the two branches in the proof of `thm:containment`. -/
...
/-- The diagonal near-normalization term in the Ritz-vector norm is at least
`-eps`; no matrix norm estimate is inserted here. -/
...
/-- In the small-loss branch, the physical Ritz vector has safely nonzero
norm.  The coarse constant `1/5` is the one used by the later Rayleigh bound. -/
...
/-- Norm consequence of `ritz_norm_sq_lower`, recorded separately so that
the constants used in the Rayleigh argument are visible in the statement. -/
...
/-- Exact Rayleigh identity obtained by taking the inner product of
`eq:ritz-residual-identity` with the physical Ritz vector. -/
...
/-- The recurrence error contributes at most the displayed norm product to
...
/-- Small-loss branch of `thm:containment`.  Its conclusion is already in
the spectral interval of `H`, enlarged by exactly the two visible error
scales used in the proof. -/
...
/-- Every leading prefix uses no more than the full-run Paige scale. -/
...
/-- Every leading prefix uses no more than the full-run recurrence scale. -/
...
/-- `thm:containment` for an arbitrary leading prefix.  Strong induction is
the formal counterpart of iterating the descent in the PDF: the large-loss
branch moves to the strictly shorter prefix supplied by `lem:descent`, while
the small-loss branch closes by `small_loss_containment`. -/
...
/-- `thm:containment` at the completed `k`-step matrix. -/
...
/-- Literal eigenvalue-enumeration form of `thm:containment`: every
eigenvalue of the computed symmetric tridiagonal lies in the explicitly
enlarged spectral interval. -/
