...
This file proves `eq:normalized-interface` and the recurrence-error part of
`lem:normalized-errors`.  The new error is defined by the two terms created
by column normalization, and its explicit bound follows from `eq:governing`
and `thm:local-errors`.
-/
...
/-- Change in the tridiagonal coefficient matrix caused by exact column
normalization. -/
noncomputable def normalizationCoeffError : Mat k k :=
  L.normDiag * L.Tmat * L.invNormDiag - L.Tmat

/-- Recurrence error after exact column normalization. -/
...
/-- Exact recurrence in `eq:normalized-interface`. -/
...
/-- Entrywise normalization perturbation, used immediately below to obtain
the operator bound. -/
...
/-- Explicit normalized recurrence-error bound supporting
`eq:normalized-interface`. -/
