...
/-!
# Exact column normalization of the computed run

This file begins the proof of `lem:normalized-errors`.  All divisions are
justified from `ass:normalization`, and their constants are explicit.
-/
...
/-- `d_j=‖q_j‖`, with Lean's zero-based matrix index. -/
...
/-- `bar V=V_k D⁻¹` preceding `eq:normalized-interface`. -/
...
/-- The columns of `bar V` are exactly unit. -/
...
/-- Strict upper normalized Gram matrix in `eq:normalized-gram`. -/
...
/-! ## The normalized boundary column -/

/-- Norm of the possible extra vector `v_{k+1}`. -/
...
/-- Unit direction of the boundary column.  When `β_k=0`, the direction is
irrelevant; choosing the first normalized Lanczos vector makes the definition
total without adding an ambient-dimension assumption. -/
noncomputable def normalizedVNext : Vec n :=
  if hb : L.β k ≠ 0 then
    (L.nextColumnNorm)⁻¹ • L.v (k + 1)
  else fun i => L.normalizedV i ⟨0, L.hk_pos⟩

/-- Nonnegative coefficient multiplying the normalized boundary direction. -/
...
/-- The assembled boundary is literally the rank-one last-column matrix. -/
...
/-- Exact normalized last-column identity used in `eq:normalized-interface`. -/
