...
/-!
# Starting spectral measure of the Greenbaum model

The optional starting-measure extension is first recorded in its most
auditable algebraic form: every moment of the completed model at its first
coordinate equals the corresponding moment of `H` at the normalized
starting vector.  For finite symmetric matrices these moments determine the
finite atomic spectral measure.
-/
...
/-- Block-diagonal embedding preserves the identity matrix. -/
...
/-- Block-diagonal embedding preserves natural powers. -/
...
/-- A block-diagonal matrix maps a vector supported in the first physical
copy to another vector supported in that copy. -/
...
/-- Inner products of vectors supported in the first physical copy reduce
to inner products of their first columns. -/
...
/-- Moment identity for the unrotated physical block operator. -/
...
/-- Powers of an orthogonal conjugate are conjugates of the powers. -/
...
/-- Orthogonal conjugation preserves every starting moment after the
starting vector is rotated by the same orthogonal matrix. -/
...
/-- The normalized physical starting vector `v₁ / ‖v₁‖`. -/
...
/-- Exact moment identity for any completion whose
first `k` columns are the physical dilation columns. -/
