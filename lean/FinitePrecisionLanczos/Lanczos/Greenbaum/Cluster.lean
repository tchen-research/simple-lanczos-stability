...
/-!
# Separated cluster masses for the Greenbaum model

This optional extension combines the
support localization, the one-Lipschitz spectral perturbation estimate, and
the direct cutoff lemma for two separated finite groups.
-/
...
/-- Product form of the cluster-mass estimate, before division by the positive
separation `g - 2 * delta`. -/
...
/-- The discrepancy between a physical spectral cluster and the
corresponding completed-model cluster is controlled by the
one-Wasserstein perturbation radius divided by their remaining gap. -/
